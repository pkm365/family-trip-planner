...
class Activity(BaseModel):
    ...
    name = Column(String(200), nullable=False, doc="Activity name or title")

    description = Column(Text, nullable=True, doc="Detailed activity description")
    ...
    activity_date = Column(
        Date, nullable=False, index=True, doc="Date when activity is planned"
    )

    time_slot = Column(Enum(TimeSlot), nullable=False, doc="Time slot for the activity")

    # Categorization
    category = Column(Enum(ActivityCategory), nullable=False, doc="Activity category")
    ...
    # Location details
    location_name = Column(String(200), nullable=True, doc="Location or venue name")

    address = Column(Text, nullable=True, doc="Activity location address")

    latitude = Column(Float, nullable=True, doc="Activity location latitude")

    longitude = Column(Float, nullable=True, doc="Activity location longitude")

    # Budget
    estimated_cost = Column(
        Float, default=0.0, nullable=False, doc="Estimated cost for this activity"
    )

    actual_cost = Column(Float, nullable=True, doc="Actual cost spent on this activity")

    # Notes
    notes = Column(Text, nullable=True, doc="Additional notes or requirements")
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<Activity(id={self.id}, name='{self.name}', date={self.activity_date})>"
        )
