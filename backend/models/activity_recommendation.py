...
class ActivityRecommendation(BaseModel):
    ...
    )  # Google Places ID, TripAdvisor ID, etc.
    external_source = Column(
        String(50), nullable=True
    )  # 'google_places', 'tripadvisor', etc.
    ...
    )  # What search led to this recommendation
    discovery_date = Column(DateTime, default=datetime.utcnow)

    # Status
    is_active = Column(
        Boolean, default=True
    )  # Can be deactivated if no longer relevant
    ...
    votes = relationship(
        "ActivityVote", back_populates="recommendation", cascade="all, delete-orphan"
    )
    comments = relationship(
        "ActivityComment", back_populates="recommendation", cascade="all, delete-orphan"
    )
    ...
    def popularity_score(self):
        ...
        external_score = (
            self.external_rating or 0
        ) * 0.2  # Weight external rating less
