...
class FamilyMember(BaseModel):
    ...
    name = Column(String(100), nullable=False, doc="Family member name")

    role = Column(Enum(MemberRole), nullable=False, doc="Family member role")

    age = Column(Integer, nullable=True, doc="Family member age (optional)")
    ...
    dietary_restrictions = Column(
        Text, nullable=True, doc="Dietary restrictions or allergies"
    )

    mobility_needs = Column(
        Text, nullable=True, doc="Mobility requirements or accessibility needs"
    )

    interests = Column(Text, nullable=True, doc="Personal interests and preferences")

    wishlist_items = Column(
        Text, nullable=True, doc="Must-see attractions or activities (JSON format)"
    )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<FamilyMember(id={self.id}, name='{self.name}', role={self.role.value})>"
        )
