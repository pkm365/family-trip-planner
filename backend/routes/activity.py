...
async def create_activity(activity_data: ActivityCreate, db: DatabaseSession) -> ActivityResponse:
    """
    ...
    try:
        ...
        if db_activity.address:
            db_activity = await trip_service.enrich_activity_with_coordinates(db, db_activity)
...
def get_activities_by_date(
    ...
    ).order_by(Activity.time_slot).all()
    ...
    for activity in activities:
        ...
        activities_by_slot[slot_key].append(
            ActivitySummary.model_validate(activity)
        )
