...
from ..schemas.activity_recommendation import ActivityRecommendationResponse
...
def create_favorite(
    favorite_data: ActivityFavoriteCreate,
    ...
    recommendation = db.query(ActivityRecommendation).filter(
        ActivityRecommendation.id == favorite_data.recommendation_id
    ).first()
    ...
    family_member = db.query(FamilyMember).filter(
        FamilyMember.id == favorite_data.family_member_id
    ).first()
    if not family_member:
        raise HTTPException(status_code=404, detail="Family member not found")
    
    # Check if favorite already exists
    existing_favorite = db.query(ActivityFavorite).filter(
...
def get_family_member_favorites(
    ...
    # Build query
    query = db.query(ActivityFavorite).filter(
    ...
    favorites = query.order_by(ActivityFavorite.favorite_date.desc()).offset(offset).limit(page_size).all()
    ...
    recommendations = db.query(ActivityRecommendation).filter(
        ActivityRecommendation.id.in_(recommendation_ids)
    ).all()
    
    # Convert recommendations to dict for easy lookup
    recommendations_dict = {rec.id: {
        "id": rec.id,
        "name": rec.name,
    ...
    recommendations_list = [recommendations_dict[fav.recommendation_id] for fav in favorites if fav.recommendation_id in recommendations_dict]
...
def update_favorite(
    ...
    favorite = db.query(ActivityFavorite).filter(ActivityFavorite.id == favorite_id).first()
    if not favorite:
...
def get_favorites_dashboard(
    ...
    for rec in recommendations:
        ...
        favorites = db.query(ActivityFavorite).filter(
            ActivityFavorite.recommendation_id == rec.id
        ).all()
        ...
        for fm in family_members:
            ...
            family_favorites.append(FamilyMemberFavoriteInfo(
        ...
        recommendations_with_favorites.append(RecommendationWithFavorites(
    ...
    favorite_statistics = {
        ...
        "average_favorites_per_recommendation": total_favorites / total_recommendations if total_recommendations > 0 else 0,
        "favorite_participation_rate": (total_favorites / (total_recommendations * len(family_members))) if total_recommendations > 0 and len(family_members) > 0 else 0
    }
