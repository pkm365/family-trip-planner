...
def create_vote(vote_data: ActivityVoteCreate, db: Session = Depends(get_db)):
    """
    ...
    recommendation = (
        ...
        .filter(ActivityRecommendation.id == vote_data.recommendation_id)
        .first()
    )
    ...
    family_member = (
        ...
        .filter(FamilyMember.id == vote_data.family_member_id)
        .first()
    )
...
def update_vote(
    vote_id: int, vote_update: ActivityVoteUpdate, db: Session = Depends(get_db)
):
...
def create_comment(comment_data: ActivityCommentCreate, db: Session = Depends(get_db)):
    """Create a comment on an activity recommendation."""
    # Check if recommendation exists
    recommendation = (
        db.query(ActivityRecommendation)
        .filter(ActivityRecommendation.id == comment_data.recommendation_id)
        .first()
    )
    if not recommendation:
        raise HTTPException(status_code=404, detail="Recommendation not found")
    ...
    family_member = (
        db.query(FamilyMember)
        .filter(FamilyMember.id == comment_data.family_member_id)
        .first()
    )
    if not family_member:
        raise HTTPException(status_code=404, detail="Family member not found")
...
def get_voting_dashboard(trip_id: int, db: Session = Depends(get_db)):
    """
    ...
    for rec in recommendations:
        ...
        votes = (
            db.query(ActivityVote)
            .filter(ActivityVote.recommendation_id == rec.id)
            .all()
        )
        ...
        comments = (
            ...
            .filter(ActivityComment.recommendation_id == rec.id)
        ...
        recommendations_with_votes.append(
            RecommendationWithVotes(
    ...
    voting_statistics = {
        ...
        "average_votes_per_recommendation": total_votes / total_recommendations
        if total_recommendations > 0
        else 0,
        "voting_participation_rate": (
            total_votes / (total_recommendations * len(family_members))
        )
