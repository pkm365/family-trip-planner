...
class GeocodingService:
    ...
    def __init__(self):
        ...
        self.geolocator = Nominatim(user_agent="family-trip-planner", timeout=10)
    ...
    async def geocode_address(self, address: str) -> Tuple[float, float]:
        ...
        try:
            ...
            location = await loop.run_in_executor(
                None, self.geolocator.geocode, address.strip()
            )
    ...
    async def reverse_geocode(self, latitude: float, longitude: float) -> str:
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            location = await loop.run_in_executor(
                None, self.geolocator.reverse, f"{latitude}, {longitude}"
            )

            if location and location.address:
                address = location.address
                logger.info(
                    f"Successfully reverse geocoded ({latitude}, {longitude}) to '{address}'"
                )
                return address
            else:
                logger.warning(
