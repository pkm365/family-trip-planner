...
class SearchService:
    ...
    async def search_activities(
        ...
        if self.google_places_api_key:
            try:
                ...
                search_metadata["results_count_by_source"]["google_places"] = len(google_results)
            ...
            async with self.session.get(text_search_url, params=params) as response:
                if response.status == 200:
                    ...
                    for place in places[:limit]:
                        ...
                        detailed_place = await self._get_place_details(place.get("place_id"))
                        if detailed_place:
            ...
            async with self.session.get(details_url, params=params) as response:
                if response.status == 200:
                    ...
                    return {
                        ...
                        "category": place.get("types", ["general"])[0] if place.get("types") else "general",
    ...
    def _generate_mock_results(self, query: str, trip_id: int, limit: int) -> List[Dict[str, Any]]:
        ...
        for activity in mock_activities:
            if (query_lower in activity["name"].lower() or 
                ...
                query_lower in activity["category"].lower()):
    ...
    async def _save_recommendations(
        ...
        for result in results:
            ...
            existing = db.query(ActivityRecommendation).filter(
            ...
            recommendation = ActivityRecommendation(
                ...
                category=result.get("category") or (
                    result.get("types", ["general"])[0] if isinstance(result.get("types"), list) else "general"
                ),
                ...
                image_urls=json.dumps(result.get("image_urls", [])) if result.get("image_urls") else None,
