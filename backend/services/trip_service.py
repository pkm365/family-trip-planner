...
class TripService:
        ...
        if not trip.accommodation_lat or not trip.accommodation_lon:
            if trip.accommodation_address:
                ...
                if lat != 0.0 and lon != 0.0:
                    ...
                    db.commit()
                    logger.info(f"Updated trip {trip.id} with coordinates ({lat}, {lon})")
        
        return trip
    
    @staticmethod
    async def enrich_activity_with_coordinates(db: Session, activity: Activity) -> Activity:
        """
        ...
        if not activity.latitude or not activity.longitude:
            if activity.address:
                ...
                if lat != 0.0 and lon != 0.0:
                    ...
                    db.commit()
                    logger.info(f"Updated activity {activity.id} with coordinates ({lat}, {lon})")
    ...
    def get_daily_activities(db: Session, trip_id: int) -> List[Dict]:
        ...
        ).order_by(Activity.activity_date, Activity.time_slot).all()
        ...
        for trip_date in TripService.get_trip_dates(trip):
            date_activities = activities_by_date.get(trip_date, {
                TimeSlot.MORNING: [],
                TimeSlot.AFTERNOON: [],
                TimeSlot.EVENING: []
            })
