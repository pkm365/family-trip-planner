...
class WeatherService:
    ...
    async def get_current_weather(self, latitude: float, longitude: float) -> Dict:
        ...
        if cache_key in self._cache and self._is_cache_valid(self._cache[cache_key]):
            logger.info(f"Returning cached current weather for ({latitude}, {longitude})")
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                logger.info(f"Successfully fetched current weather for ({latitude}, {longitude})")
                return data
    ...
    async def get_weather_forecast(self, latitude: float, longitude: float, days: int = 5) -> Dict:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                logger.info(f"Successfully fetched {days}-day forecast for ({latitude}, {longitude})")
                return data
