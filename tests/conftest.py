...
def mock_weather_service():
    ...
    mock_service.get_weather_forecast = AsyncMock(
        return_value={
            "list": [
                {
                    "dt": 1627776000,
    ...
    mock_service.format_weather_summary = Mock(
        return_value={
            "temperature": 26,
            "feels_like": 27,
            "humidity": 65,
    ...
    mock_service.get_daily_forecasts = Mock(
        return_value=[
            {
                "date": "2025-07-27",
                "min_temp": 22,
...
def assert_datetime_close(dt1, dt2, delta_seconds=5):
    ...
    assert (
        diff <= delta_seconds
    ), f"Datetimes differ by {diff} seconds (max: {delta_seconds})"
...
def assert_coordinates_close(coord1, coord2, delta=0.001):
    ...
    assert (
        abs(lat1 - lat2) <= delta
    ), f"Latitudes differ by {abs(lat1 - lat2)} (max: {delta})"
    assert (
        abs(lon1 - lon2) <= delta
    ), f"Longitudes differ by {abs(lon1 - lon2)} (max: {delta})"
